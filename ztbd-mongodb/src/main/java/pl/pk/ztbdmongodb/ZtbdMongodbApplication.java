...
public class ZtbdMongodbApplication implements ApplicationRunner {
  ...
  public void run(ApplicationArguments args) throws IOException {
    ...
    if (collections != null) {
      List<String> results =
          List.of(
              importService.importTo(
                  collections.get(0), ImportUtils.linesFromResource(files.get(0))),
              importService.importTo(
                  collections.get(1), ImportUtils.linesFromResource(files.get(1))),
              importService.importTo(
                  collections.get(2), ImportUtils.linesFromResource(files.get(2))));
